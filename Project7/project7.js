...
function SimTimeStep( dt, positions, velocities, springs, stiffness, damping, particleMass, gravity, restitution )
{
	...
	{
		const obj1Index = springs[i].p0;
		const obj2Index = springs[i].p1;
		const restLen = springs[i].rest;
		const l = positions[obj2Index].sub(positions[obj1Index]).len();
		const d = positions[obj2Index].sub(positions[obj1Index]).mul(1.0/l);
		const springForce = d.mul(stiffness * (l - restLen));
		forces[obj1Index] = forces[obj1Index].add(springForce);
		forces[obj2Index] = forces[obj2Index].sub(springForce);

		const acel = forces[obj1Index].mul(1/particleMass);
		const newVel = velocities[obj1Index].add(acel.mul(dt));
		const ldot = newVel.sub(velocities[obj1Index]).dot(d);
		const dampForce = d.mul(damping * -ldot);

		forces[obj1Index] = forces[obj1Index].add(dampForce);
		forces[obj2Index] = forces[obj2Index].sub(dampForce);
	...
	for(let i = 0; i < forces.length; i++)
	{
		const acel = forces[i].mul(1/particleMass);
		velocities[i] = velocities[i].add(acel.mul(dt));
		positions[i] = positions[i].add(velocities[i].mul(dt));
		if(positions[i].y < -1)
		{
			const heighDif = -1 - positions[i].y;
			positions[i].y = -1 + restitution * heighDif;
